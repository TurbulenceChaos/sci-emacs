...
#+begin_src bash :noeval
  ...
  # Build emacs-jupyter
  cd ~/.emacs.d/lisp-site/jupyter
  make

  # Build magit
  cd ~/.emacs.d/lisp-site/magit/lisp
  make

  # Return to the Emacs directory
  cd ~/.emacs.d

  # Byte compile Emacs Lisp directories excluding 'jupyter' and 'magit'
  emacs --batch --eval "
  (let ((excluded '(\"jupyter\" \"magit\"))
        (base-dir \"~/.emacs.d/lisp-site/\"))
    (dolist (dir (directory-files base-dir t \"^[^.].*\"))
      (when (and (file-directory-p dir)
                 (not (member (file-name-nondirectory dir) excluded)))
